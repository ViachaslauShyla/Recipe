...
NSString *const kOAIApplicationJSONType = @"application/json";

NSString * OAIPercentEscapedStringFromString(NSString *string)
{
    static NSString *const kOAICharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString *const kOAICharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet *allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kOAICharactersGeneralDelimitersToEncode stringByAppendingString:kOAICharactersSubDelimitersToEncode]];
...
- (id)sanitizeForSerialization:(id)object {
    if (object == nil) {
        return nil;
    } else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[OAIQueryParamCollection class]]) {
        return object;
    } else if ([object isKindOfClass:[NSDate class]]) {
        return [OAISanitizer dateToString:object];
        ...
        return sanitizedObjs;
    } else if ([object isKindOfClass:[OAIObject class]]) {
...
- (NSString *)parameterToString:(id)param {
    if ([param isKindOfClass:[NSString class]]) {
        ...
        return [OAISanitizer dateToString:param];
...
+ (NSString *)dateToString:(id)date {
    NSTimeZone *timeZone = [OAIDefaultConfiguration sharedConfig].serializationTimeZone;
...
- (NSString *)selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kOAIApplicationJSONType;
        }
